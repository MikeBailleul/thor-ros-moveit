...
ThorHardwareInterface::ThorHardwareInterface() {
    ...
    for (std::size_t i = 0; i < joint_names.size(); ++i) {
        ...
        hardware_interface::JointHandle pos_handle(joint_state_interface.getHandle(joint_names[i]), &cmd_pos[i]);
...
void ThorHardwareInterface::read() {
    ...
    for (std::size_t i = 0; i < joint_names.size(); ++i) {
        ROS_INFO_STREAM("Pos for " << joint_names[i].c_str() << ": " << cmd_pos[i]);
...
void ThorHardwareInterface::write() {
    ...
    for (std::size_t i = 0; i < joint_names.size(); ++i) {
        ROS_INFO_STREAM("Command for " << joint_names[i].c_str() << ": " << cmd_pos[i]);
    }
}
